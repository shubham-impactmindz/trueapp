...
object BigBannerRepository {
    private val webService = ApiHelper.createService()
     fun bigBanners(
         successHandler: (BigBannersResponse) -> Unit,
         failureHandler: (String) -> Unit,
         onFailure: (Throwable) -> Unit,
         token: String,
    ) {
        webService.fetchBigBanners(token)
            .enqueue(object : Callback<BigBannersResponse> {
                override fun onResponse(
                    call: Call<BigBannersResponse>,
                    response: Response<BigBannersResponse>
                ) {
                    response.body()?.let {
                        successHandler(it)
                    }
                ...
                override fun onFailure(call: Call<BigBannersResponse>, t: Throwable) {
                    onFailure(t)
                }
            })
    }
