...
object RoundBannerRepository {
    private val webService = ApiHelper.createService()
     fun roundBanners(
         successHandler: (RoundBannersResponse) -> Unit,
         failureHandler: (String) -> Unit,
         onFailure: (Throwable) -> Unit,
         token: String,
    ) {
        webService.fetchRoundBanners(token)
            .enqueue(object : Callback<RoundBannersResponse> {
                override fun onResponse(
                    call: Call<RoundBannersResponse>,
                    response: Response<RoundBannersResponse>
                ) {
                    response.body()?.let {
                        successHandler(it)
                    }
                ...
                override fun onFailure(call: Call<RoundBannersResponse>, t: Throwable) {
                    onFailure(t)
                }
            })
    }
