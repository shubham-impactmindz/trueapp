...
object SmallBannerRepository {
    private val webService = ApiHelper.createService()
     fun smallBanners(
         successHandler: (SmallBannersResponse) -> Unit,
         failureHandler: (String) -> Unit,
         onFailure: (Throwable) -> Unit,
         token: String,
    ) {
        webService.fetchSmallBanners(token)
            .enqueue(object : Callback<SmallBannersResponse> {
                override fun onResponse(
                    call: Call<SmallBannersResponse>,
                    response: Response<SmallBannersResponse>
                ) {
                    response.body()?.let {
                        successHandler(it)
                    }
                ...
                override fun onFailure(call: Call<SmallBannersResponse>, t: Throwable) {
                    onFailure(t)
                }
            })
    }
